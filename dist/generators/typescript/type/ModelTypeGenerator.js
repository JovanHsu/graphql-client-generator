 ...
 */
class ModelTypeGenerator extends BaseTypeScriptGenerator_1.BaseTypeScriptGenerator {
     ...
     * 生成模型类型内容
     */
    generateModelTypeContent(types) {
        if (types.length === 0) {
        ...
        if (dependencies.size > 0) {
            ...
            const importTypes = Array.from(allDependencies).filter(dep => !types.some(type => type.name === dep));
    ...
    async generate() {
        ...
        // 如果有多个文件，生成索引文件
        if (batches.length > 1) {
            const indexContent = Array.from({ length: batches.length }, (_, i) => `export * from './types.models.${i + 1}';`).join('\\n');
