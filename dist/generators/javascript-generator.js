...
`;
    ...
    async generateQueries() {
        const content = this.generateOperationsFile(this.schema.operations.queries, 'Query', 'query');
    ...
    async generateMutations() {
        const content = this.generateOperationsFile(this.schema.operations.mutations, 'Mutation', 'mutation');
    ...
    async generateSubscriptions() {
        const content = this.generateOperationsFile(this.schema.operations.subscriptions, 'Subscription', 'subscription');
...
export class GraphQLApiClient {
  ...
  get subscription() {
     ...
     * 生成操作文件内容
     */
    generateOperationsFile(operations, clientName, operationType) {
 ...
 */`;
    ...
    generateOperationMethod(operation) {
        ...
        const jsdocParams = operation.arguments.map(arg => ` * @param {${this.convertType(arg.type)}} ${arg.name}${arg.required ? '' : '='}`);
        return `  /**
   * ${methodName} 操作
${jsdocParams.join('\n')}
  ...
  async ${methodName}(${operation.arguments.map(arg => arg.name).join(', ')}) {
    ...
    generateTypeDefinition(name, fields) {
        const fieldDefinitions = fields.map(field => ` * @property {${this.convertType(field.type)}} ${field.name}${field.required ? '' : '='}`);
        return `/**
 ...
 */`;
    ...
    generateOperation(operation) {
        const args = operation.arguments.map(arg => `${arg.name}${arg.required ? '' : '='}`);
        const jsdocParams = operation.arguments.map(arg => ` * @param {${this.convertType(arg.type)}} ${arg.name}${arg.required ? '' : '='}`);
        return `  /**
   * ${operation.name} 操作
${jsdocParams.join('\n')}
