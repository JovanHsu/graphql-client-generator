 ...
 */
export class InputTypeGenerator extends BaseTypeScriptGenerator {
  constructor(
   ...
   * 生成输入类型内容
   */
  private generateInputTypeContent(inputs: TypeInfo[]): string {
    if (inputs.length === 0) {
    ...
    for (const input of inputs) {
      ...
      // 处理字段和依赖
      const processedFields = input.fields?.map((field: any) => {
    ...
    if (dependencies.size > 0) {
      const importTypes = Array.from(dependencies).filter(dep => 
        !inputs.some(input => input.name === dep)
      );
